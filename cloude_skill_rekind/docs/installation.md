...
Dokumen ini merupakan panduan resmi instalasi dan konfigurasi awal (*setup*) repository, agar setiap developer dapat menjalankan project dengan benar sejak penggunaan pertama.

| Atribut | Keterangan |
|---|---|
| Jenis dokumen | Panduan instalasi dan setup |
| Ruang lingkup | Persiapan lingkungan kerja lokal untuk pengembangan project AI Agent |
...
> **Catatan mengenai versi perangkat lunak.** Tampilan antarmuka dan tautan unduhan perangkat lunak pihak ketiga dapat berubah sewaktu-waktu. Apabila terdapat perbedaan antara dokumen ini dan tampilan sebenarnya, ikuti dokumentasi resmi perangkat lunak yang bersangkutan, kemudian laporkan perbedaan tersebut kepada pemilik dokumen.
...
|---:|---|---|
| 1 | Kesetaraan lingkungan kerja | Seluruh kontributor bekerja pada lingkungan yang setara sehingga hasil pekerjaan dapat direproduksi. |
...
> **Note.** Kebutuhan memori sebesar 16 GB dianjurkan apabila developer menjalankan VS Code, Claude Desktop, dan peramban secara bersamaan.
...
> **Warning.** Seluruh perangkat lunak wajib diunduh dari situs resmi masing-masing. Pengunduhan dari sumber tidak resmi berisiko menyisipkan perangkat berbahaya ke dalam komputer kerja.
...
3. Pada tahap pemilihan editor, pilih Visual Studio Code apabila telah terpasang.
...
2. Jalankan perintah `git --version`. Apabila Git belum terpasang, sistem akan menawarkan pemasangan Command Line Tools.
...
3. Khusus Windows, aktifkan opsi penambahan VS Code ke `PATH` apabila tersedia, agar perintah `code` dapat dijalankan dari terminal.
...
> **Note.** Apabila perintah `code` belum dikenali pada macOS, buka VS Code, tekan `Cmd + Shift + P`, lalu jalankan perintah "Shell Command: Install 'code' command in PATH".
...
> **Note.** Ketersediaan Claude Desktop untuk setiap sistem operasi dapat berubah. Apabila paket pemasang untuk sistem operasi yang digunakan tidak tersedia, gunakan Claude melalui peramban web dan laporkan kondisi tersebut kepada Repository Maintainer.
...
Struktur berikut merupakan pola umum repository kumpulan skill dan agent.
...
> **Note.** Struktur di atas merupakan pola umum. Struktur resmi repository dapat berbeda dan mengikuti ketetapan Repository Maintainer.
...
> **Warning.** Direktori `.git/` tidak boleh disunting, dipindahkan, atau dihapus secara manual. Tindakan tersebut dapat merusak seluruh riwayat repository pada komputer lokal.
...
> **Warning.** Pengembangan tidak boleh dilakukan langsung pada *branch* utama. Seluruh perubahan dikerjakan pada *branch* kerja dan digabungkan melalui *pull request*.
...
| Jenis berkas | Alasan |
|---|---|
| Berkas ZIP hasil generasi | Merupakan berkas sementara, bukan hasil akhir |
...
Hasil generasi Claude berkedudukan sebagai **draf**. Berkas ZIP merupakan sarana pemindahan, bukan berkas yang disimpan di dalam repository.
...
- [ ] Tidak terdapat berkas sementara yang ikut tersalin.
- [ ] Tidak terdapat kredensial atau data rahasia di dalam berkas.
...
Instalasi dinyatakan berhasil apabila seluruh pemeriksaan berikut memberikan hasil yang diharapkan.
...
| Gagal *push* karena riwayat tertinggal | Terdapat perubahan baru pada *remote* | Jalankan `git pull --rebase`, selesaikan konflik, lalu ulangi *push* |
...
| Ekstensi gagal dipasang | Koneksi internet terbatas atau kebijakan jaringan | Periksa koneksi, lalu hubungi unit teknologi informasi apabila akses diblokir |
...
Apabila permasalahan tidak dapat diselesaikan melalui tabel di atas, kontributor menyampaikan laporan yang memuat gejala, langkah yang telah dilakukan, pesan galat, serta sistem operasi dan versi perangkat lunak yang digunakan.
...
Seluruh butir berikut wajib terpenuhi sebelum pengembangan dimulai.
...
- [ ] GitHub CLI telah terpasang, apabila digunakan.
...
- [ ] Ketentuan bahwa hasil generasi Claude merupakan draf yang wajib ditinjau telah dipahami.
...
> **Note.** Bagi mahasiswa magang, seluruh butir di atas diverifikasi bersama pembimbing sebelum kontribusi pertama diajukan.
...
| **Repository** | Tempat penyimpanan berkas project beserta seluruh riwayat perubahannya. |
