...
> [!WARNING]
> Seluruh pembelajaran dalam dokumen ini berstatus **wajib diikuti** dalam pengembangan repository. Penyimpangan terhadap approved lesson hanya diperkenankan melalui pengajuan pengecualian tertulis kepada Repository Maintainer disertai alasan dan batas waktu berlakunya.

> [!NOTE]
> Dokumen ini merupakan standar pada tingkat repository. Dokumen ini **bukan** kebijakan resmi perusahaan dan tidak menggantikan pedoman, SOP, atau instruksi kerja yang diterbitkan oleh Business Excellence Center. Apabila terdapat pertentangan dengan dokumen resmi BEC, dokumen resmi BEC yang berlaku.
...
Approved lessons adalah repositori resmi seluruh pembelajaran yang telah divalidasi, disetujui, dan ditetapkan sebagai standar dalam pengembangan AI Agent, Skill Claude, dokumentasi, maupun repository.
...
6. **Menjaga konsistensi lintas agent dan lintas skill.** Standar yang sama diterapkan pada seluruh komponen repository.
...
Sebuah pembelajaran hanya dapat diangkat menjadi approved lesson apabila memenuhi **seluruh** kriteria wajib berikut.

### 2.1 Kriteria Wajib

- [ ] **Telah berstatus `V3 — Tervalidasi`** pada `learning-log.md` sekurang-kurangnya satu siklus reviu bulanan.
- [ ] **Didukung sekurang-kurangnya tiga bukti terdokumentasi**, dengan sekurang-kurangnya satu bukti berasal dari konteks yang berbeda dari kasus asalnya.
...
- [ ] **Telah disosialisasikan** kepada seluruh anggota tim sebelum diberlakukan.

### 2.2 Kriteria Penguat

Kriteria berikut tidak bersifat wajib, namun memperkuat kelayakan sebuah pembelajaran untuk disetujui.

- [ ] Terdapat data kuantitatif yang menunjukkan perbaikan terukur.
...
- [ ] Terdapat anggota tim yang bersedia menjadi penanggung jawab pemeliharaannya.

### 2.3 Kriteria Penolakan

Pengajuan **wajib ditolak** apabila ditemukan salah satu kondisi berikut.
...
Seluruh approved lesson berasal dari `learning-log.md`. Namun learning log sendiri memperoleh pembelajaran dari beragam sumber, dan kekuatan sumber tersebut memengaruhi kelayakan pengajuan.

| Kode | Sumber | Kekuatan bukti | Dapat langsung diajukan? |
|---|---|---|---|
| `SRC-COR` | Correction log yang telah ditutup dan diverifikasi | Tinggi | Ya, apabila kriteria terpenuhi |
| `SRC-EXP` | Eksperimen terkendali dengan pembanding | Tinggi | Ya, apabila kriteria terpenuhi |
| `SRC-EVL` | Evaluasi berkala dengan kumpulan kasus uji baku | Tinggi | Ya, apabila kriteria terpenuhi |
| `SRC-DES` | Keputusan desain arsitektur beserta konsekuensinya | Sedang | Ya, apabila disertai bukti hasil penerapan |
...
4. **Cari bukti sanggahan secara aktif.** Telusuri kasus yang seharusnya sesuai namun ternyata tidak. Hasil penelusuran wajib dicatat, termasuk apabila tidak ditemukan sanggahan.
...
7. **Uji keterperiksaan.** Minta pihak lain menilai satu contoh perubahan menggunakan rumusan pembelajaran tersebut. Apabila penilaiannya berbeda dari penilaian PIC, rumusan masih terlalu kabur.
...
- [ ] Terdapat minimal tiga bukti terdokumentasi
...
2. **Pemeriksaan kelengkapan.** Repository Maintainer memeriksa pemenuhan seluruh kriteria wajib pada Bagian 2.1.
...
5. **Reviu terbuka.** Rancangan diedarkan kepada seluruh anggota tim selama lima hari kerja untuk memperoleh keberatan atau masukan.
...
9. **Sosialisasi.** Approved lesson diumumkan kepada seluruh anggota tim beserta tanggal mulai berlakunya.
...
> [!WARNING]
> PIC pembelajaran **tidak boleh** menjadi approver atas pengajuannya sendiri. Apabila Repository Maintainer merupakan PIC pengajuan, kewenangan approval dilimpahkan kepada anggota senior lain yang ditunjuk secara tertulis.
...
> [!WARNING]
> Seluruh approved lesson berkategori `SAF` ditetapkan minimal berprioritas `A1`. Pengecualian atas lesson `A1` wajib memiliki tenggat waktu dan tidak boleh bersifat permanen.
...
5. Angka hasil pengukuran dicantumkan apabila tersedia, disertai ukuran sampelnya.
6. Seluruh contoh keluaran menggunakan data buatan, bukan cuplikan sesi nyata.
...
flowchart TD
    ...
    O --> P{Seluruh kriteria wajib<br/>terpenuhi?}
...
- [ ] Apakah terdapat bukti baru yang membantah pembelajaran ini?
- [ ] Apakah terdapat pengecualian yang diberikan lebih dari dua kali? Bila ya, batas keberlakuan perlu ditinjau.
...
> [!WARNING]
> Pengecualian yang sama diajukan tiga kali atau lebih merupakan pertanda bahwa approved lesson tersebut tidak realistis. Kondisi ini **wajib** memicu peninjauan ulang atas rumusan atau batas keberlakuannya, bukan penolakan berulang atas pengajuan.
...
> [!WARNING]
> Praktik berikut membuat approved lessons berubah menjadi daftar aturan yang diabaikan seluruh anggota tim.

| Kesalahan | Mengapa bermasalah | Yang seharusnya dilakukan |
|---|---|---|
| Menyetujui pembelajaran hanya berdasar satu kasus | Standar yang rapuh akan dibantah dan merusak kepercayaan pada seluruh dokumen | Penuhi kriteria minimal tiga bukti dari konteks berbeda |
...
| Melompati learning log | Pembelajaran belum teruji namun sudah diwajibkan | Seluruh entri wajib melewati status `V3` terlebih dahulu |
...
Seluruh entri berikut merupakan ilustrasi berdasarkan situasi umum pengembangan repository AI Multi-Agent dan pengembangan Custom Skill Claude. Entri ini bersifat **ilustratif** dan bukan catatan persetujuan resmi.
...
| AL-SAF-002 | Contoh keluaran pada dokumentasi wajib menggunakan data buatan | 2026-07-20 | Dokumentasi internal banyak memuat contoh keluaran agent untuk memperjelas penjelasan | Beberapa contoh diambil langsung dari sesi nyata dan memuat penggalan isi dokumen internal | Seluruh contoh diganti data buatan yang menyerupai, disertai penandaan eksplisit bahwa contoh bersifat ilustratif | Dokumentasi cenderung tersebar lebih luas daripada perkiraan penulisnya | Seluruh contoh pada tiga dokumen utama telah bersih dari cuplikan sesi nyata | Tandai setiap contoh sebagai ilustrasi; periksa keberadaan cuplikan nyata pada setiap reviu dokumen | Approved | Repo Maintainer-DL |
...
Seluruh agent pada repository ini bertugas menjawab berdasarkan dokumen sumber yang tersedia. Persona awal telah memuat instruksi umum berupa larangan mengarang informasi dan kewajiban merujuk dokumen. Instruksi tersebut dinilai memadai karena bersifat menyeluruh.
...
Instruksi negatif yang bersifat umum lemah terhadap informasi yang memiliki pola teratur, karena melengkapi pola merupakan perilaku alami model bahasa dan tidak dikenali sebagai pelanggaran atas larangan umum. Larangan harus dirinci pada kategori informasi yang paling mudah ditebak polanya, dan setiap larangan wajib disertai alternatif tindakan yang jelas.

**Batas keberlakuan**

Berlaku pada seluruh agent yang menjawab berdasarkan dokumen sumber. Tidak berlaku pada agent yang memang bertugas menghasilkan gagasan atau rancangan, misalnya agent penyusun draf, sepanjang keluarannya ditandai secara jelas sebagai usulan dan bukan kutipan dokumen.
...
- [ ] Rinci larangan pada setiap kategori informasi berpola pada seluruh persona baru.
...
| **Multi-agent** | Arsitektur yang menggunakan beberapa AI agent dengan peran berbeda dan saling berkoordinasi. |
...
Approved lessons hanya bernilai apabila dibuka sebelum pekerjaan dimulai, bukan sekadar dirujuk ketika terjadi perselisihan. Repository Maintainer dianjurkan menjadikan dokumen ini sebagai rujukan pertama pada setiap perencanaan perubahan besar, dan seluruh reviewer wajib menyebut ID approved lesson yang menjadi dasar ketika menolak sebuah perubahan.

Kekuatan dokumen ini terletak pada jumlahnya yang terjaga tetap sedikit. Setiap penambahan entri baru sebaiknya disertai pertimbangan apakah terdapat entri lama yang telah dapat dipensiunkan.
