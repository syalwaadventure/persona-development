...
**Dokumen Standar Pencatatan Koreksi Repository AI Agent**
...
Correction log adalah catatan resmi seluruh kesalahan, koreksi, bug, dan perbaikan yang ditemukan selama proses pengembangan AI Agent, Skill Claude, maupun dokumentasi repository.
...
Correction log **wajib** dibuat apabila ditemukan salah satu kondisi berikut.

- [ ] AI Agent menghasilkan jawaban yang salah, mengarang informasi (*halusinasi*), atau menyebut dokumen yang tidak ada.
...
- [ ] Terdapat pertentangan antara isi skill, persona, dan dokumen sumber.
...
- [ ] Terdapat tautan rusak, nama berkas salah, atau struktur folder yang tidak konsisten.
...
| **PIC (Person in Charge)** | Melakukan analisis penyebab, menuliskan solusi, melaksanakan perbaikan, dan memperbarui status hingga `Fixed`. |
...
| **Business Excellence Center (BEC)** | Memberikan klarifikasi atas isi dokumen proses bisnis apabila kesalahan berkaitan dengan substansi dokumen resmi. BEC bukan pelaksana perbaikan teknis. |
...
|---|---|---|---|---|
| `P1` | Kritis | Agent memberi informasi yang berpotensi menyesatkan keputusan, atau terdapat risiko kebocoran data | Segera (≤ 2 jam kerja) | ≤ 1 hari kerja |
| `P2` | Tinggi | Fungsi utama tidak berjalan, atau kesalahan akurasi yang berulang | ≤ 1 hari kerja | ≤ 3 hari kerja |
| `P3` | Sedang | Fungsi berjalan namun terdapat penyimpangan yang mengganggu | ≤ 3 hari kerja | ≤ 10 hari kerja |
| `P4` | Rendah | Ketidaksempurnaan kecil, kosmetik, atau perbaikan redaksional | Terjadwal | Pada siklus perawatan berikutnya |

> [!WARNING]
> Seluruh temuan berkategori `SEC` (Keamanan/Kerahasiaan) ditetapkan minimal sebagai `P1` tanpa memandang besar kecilnya dampak yang terlihat. Detail teknis kebocoran **tidak boleh** dituliskan pada correction log; cukup rujuk nomor tiket internal.
...
> [!NOTE]
> Nomor ID **tidak pernah digunakan ulang**, bahkan apabila entri berstatus `Duplicate` atau `Won't Fix`. Nomor yang telah terpakai bersifat permanen.
...
1. **Konfirmasi ulang temuan.** Ulangi langkah yang menimbulkan masalah sekurang-kurangnya satu kali untuk memastikan bahwa masalah dapat direproduksi.
2. **Cari duplikat.** Telusuri correction log dengan kata kunci terkait. Apabila entri serupa telah ada, tambahkan catatan pada entri tersebut alih-alih membuat entri baru.
...
**Jawaban singkat:** verifikasi dilakukan oleh pihak yang berbeda dari PIC, dengan mengulang skenario asli dan sekurang-kurangnya satu skenario terkait untuk memastikan tidak ada regresi.
...
4. **Periksa kesesuaian dokumentasi.** Apabila perbaikan mengubah perilaku, pastikan dokumentasi terkait ikut diperbarui.
...
- [ ] Sekurang-kurangnya dua skenario variasi telah diuji
...
**Jawaban singkat:** entri ditutup oleh Repository Maintainer setelah status `Verified` tercapai, seluruh berkas terkait telah masuk ke *branch* utama, dan pelajaran yang relevan telah dipindahkan ke learning log.
...
3. **Periksa kelengkapan kolom.** Seluruh kolom wajib terisi, termasuk penyebab, solusi, dan tanggal penyelesaian.
4. **Evaluasi nilai pembelajaran.** Apabila entri mengandung pelajaran yang berlaku umum, buat entri padanan pada `learning-log.md` dan rujuk silang ID-nya.
...
- [ ] Seluruh kolom wajib terisi tanpa placeholder
...
| Pembaca utama | Developer dan AI Engineer | Seluruh tim, termasuk anggota baru |
...
7. **Gunakan penyebab akar, bukan penyebab permukaan.** Telusuri hingga sekurang-kurangnya dua tingkat pertanyaan "mengapa".
...
10. **Lakukan reviu bulanan.** Repository Maintainer meninjau seluruh entri terbuka dan menyarikan pola berulang.
...
Contoh berikut merupakan ilustrasi berdasarkan situasi umum pengembangan repository AI Agent. Contoh ini bersifat **ilustratif** dan bukan catatan kejadian resmi.
...
| CL-REP-202606-003 | 2026-06-18 | `docs/` | Seluruh tautan relatif pada `README.md` menuju dokumentasi mengembalikan halaman tidak ditemukan | Folder `documentation/` diganti nama menjadi `docs/` tanpa memperbarui tautan yang merujuknya | Pembaca baru dan mahasiswa magang tidak dapat mengakses panduan onboarding | Seluruh tautan diperbarui; ditambahkan skrip pemeriksa tautan pada alur kerja CI | Closed | Developer-AP | 2026-06-19 |
...
| CL-SCR-202607-002 | 2026-07-09 | `scripts/validate-skills.sh` | Skrip validasi skill gagal dengan pesan `unexpected token` ketika nama folder skill mengandung spasi | Variabel path pada skrip tidak diapit tanda kutip, sehingga nama berkas terpecah pada karakter spasi | Alur kerja CI gagal seluruhnya; tiga *pull request* tertahan selama satu hari | Seluruh variabel path diapit tanda kutip ganda; ditambahkan kasus uji berisi nama folder berspasi | Closed | Developer-AP | 2026-07-09 |
...
Agent menyebut nomor SOP yang tidak terdapat pada dokumen mana pun, dengan nada penyampaian yang meyakinkan.
...
| **Persona** | Kumpulan instruksi yang menetapkan identitas, gaya komunikasi, cakupan, dan batasan wewenang sebuah AI agent. |
| **Penyebab akar (root cause)** | Penyebab paling mendasar yang, apabila diperbaiki, mencegah masalah terulang. |
...
| **Project Knowledge** | Kumpulan dokumen sumber yang menjadi rujukan utama jawaban AI agent. |

---

## Catatan Penutup

Correction log hanya bernilai apabila diisi secara konsisten dan tepat waktu. Dokumen ini menetapkan standar minimum; setiap tim dapat menambahkan kolom atau kategori sesuai kebutuhan, sepanjang struktur ID, status, dan prosedur verifikasi tetap dipertahankan agar rekapitulasi antar periode tetap dapat dibandingkan.
