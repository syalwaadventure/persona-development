...
- Seluruh tombol (`button`) dapat dijangkau dengan `Tab` dan diaktifkan dengan `Enter`/`Spasi` — gunakan elemen `<button>`, jangan `<div onclick>`.
...
**Uji cepat:** bila seluruh gambar dan warna dihapus, apakah isi buku masih dapat dipahami? Bila tidak, ada informasi yang hanya tersimpan di elemen visual.
