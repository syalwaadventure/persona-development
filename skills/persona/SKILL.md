...
|---|---|---|
| **A — Persona baru** | Pengguna ingin membuat persona dari nol dan ingin review bertahap | `persona-components.md`, `agent-type-taxonomy.md`, `persona-templates.md`, `authority-and-safety.md` |
| **A2 — Fast Build** | Pengguna meminta langsung jadi atau input sudah lengkap | referensi Mode A, tetapi brief dan specification dibuat dalam satu keluaran dengan asumsi terlihat |
...
5. Batas kewenangan.
6. Bahasa dan tingkat formalitas.
7. Format keluaran yang diharapkan.

Bahasa tidak perlu ditanyakan bila pengguna sudah menyebutkannya atau konteks
output sangat jelas. Jika belum jelas, gunakan bahasa percakapan sebagai asumsi
yang terlihat dan tawarkan penyesuaian; tanyakan hanya bila kesalahan bahasa
akan mengubah hasil secara material.
...
**Aturan bertanya:** maksimal 3 pertanyaan per putaran, terarah, dan tidak
menanyakan ulang hal yang sudah jelas. Bila tujuan, pengguna, dan domain sudah
tersedia, lanjutkan dengan asumsi bertanda. Gunakan Mode A2 ketika pengguna
meminta hasil langsung atau paket input sudah cukup lengkap.
...
Ringkasan satu halaman menggunakan template di `references/persona-templates.md`.
Pada Mode A, tampilkan brief dan minta persetujuan sebelum spesifikasi penuh.
Pada Mode A2 Fast Build, tampilkan brief ringkas dan langsung lanjut ke
specification dalam keluaran yang sama; tandai asumsi dan status `draft`.
...
## 2A. Routing dan batas dengan skill lain

| Permintaan utama | Routing |
|---|---|
| Membuat atau mengaudit perilaku agent | `persona` |
| Menjalankan analisis model bisnis/financial feasibility | `indonesia-business-modeler` |
| Menilai maturity Business Excellence | `business-excellence-assessor` |
| Memantau aksi korporasi/sector intelligence | `indonesia-corporate-action-intelligence` |
| Menulis buku/handbook HTML | `book-writer` |

Persona hanya mengatur **cara agent berperilaku**. Metodologi domain, formula,
framework, source registry, dan template analisis tetap berada pada skill domain.
Jangan menyalin seluruh isi skill domain ke persona; cukup rujuk dan tetapkan
batas pemakaiannya.

## 2B. Definition of complete persona

Persona dinyatakan lengkap secara struktural hanya jika:

- 12 blok terisi atau memiliki placeholder eksplisit;
- trigger dan non-trigger jelas;
- source rules dapat dilaksanakan;
- authority level dan escalation path jelas;
- failure handling mencakup missing data, conflict, out-of-scope, tool failure,
  dan sensitive information;
- response format memiliki default dan variasi untuk jawaban sederhana;
- minimal 3 input-output examples;
- acceptance criteria dan tests tersedia;
- assumptions, unresolved decisions, risk, version, dan status tercatat.

Lengkap secara struktural tidak berarti sudah tervalidasi secara operasional.
...
Versi skill: **1.0.0-draft** dalam package **0.6.0-draft**. Static validation
bukan behavioral validation; test aktual tetap harus dijalankan.
